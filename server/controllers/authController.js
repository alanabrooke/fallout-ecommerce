  ...
  async function login(req, res) {
    ...
    return res.status(409).json('User not found. Please register as a new user before logging in.');
      ...
      const user = await bcrypt.compareSync(password, foundUser[0].password);
        req.session.user = { user_id: user.user_id, username: user.username, is_vendor: user.is_vendor, email: user.email};
        res.status(200).json(req.session.user);
    }
  }
...
async function register(req,res) {
    ...
    req.session.user = { user_id: user.user_id, username: user.username, is_vendor: user.is_vendor, email: user.email, address: user.address,
       phone: user.phone}
